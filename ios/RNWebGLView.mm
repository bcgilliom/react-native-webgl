...
- (void)setOnSurfaceCreate:(RCTDirectEventBlock)onSurfaceCreate
{
  ...
  if (_onSurfaceCreate) {
    ...
    if (!bridge.executorClass || [NSStringFromClass(bridge.executorClass) isEqualToString:@"RCTJSCExecutor"]) {
      // On JS thread, extract JavaScriptCore context, create RNWebGL context, call JS callback
      ...
      [bridge dispatchBlock:^{
        ...
        _ctxId = RNWebGLContextCreate(jsContextRef);
        _onSurfaceCreate(@{ @"ctxId": @(_ctxId) });
      } queue:RCTJSThread];
    } else {
      RCTLog(@"RNWebGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/react-native/docs/debugging.html)? RNWebGL is not supported while using Remote Debugging, you will need to disable it to use RNWebGL.");
    }
  }
}
...
- (void)draw
{
  // _ctxId may be unset if we get here (on the UI thread) before RNWebGLContextCreate(...) is
  // called on the JS thread to create the RNWebGL context and save its id (see init method). In
